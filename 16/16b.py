...
def run_experiment(beam):
    for (row_index, row) in enumerate(rows):
        for (column_index, tile) in enumerate(row):
            tile.energized = False
            tile.already_split = False
...
for i in range(len(rows)):
    max_found = max(max_found, run_experiment(Beam(0, i, Direction.EAST)))
    max_found = max(max_found, run_experiment(Beam(len(rows[0]) - 1, i, Direction.WEST)))
for j in range(len(rows[0])):
    max_found = max(max_found, run_experiment(Beam(j, 0, Direction.SOUTH)))
    max_found = max(max_found, run_experiment(Beam(j, len(rows) - 1, Direction.NORTH)))
